...
- 当前状态：官方 README 与 `SKILL.md` 已核验。共享仓库只记录能力、边界和官方来源，不代表接收者机器已安装；调用前必须重新检查安装、版本、依赖与权限。普通 PPT 任务优先使用接收者已有的通用制作能力；证据充足、高信息密度、需要 SCR 与逐页严格验收的咨询式任务才路由到 CyberPPT。
