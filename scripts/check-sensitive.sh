...
if command -v rg >/dev/null 2>&1; then
  if rg -n -i "$pattern" "$repo_root" --glob '!.git/**' --glob '!scripts/check-sensitive.sh' --glob '!examples/feishu-content-inbox/.env.example'; then
    echo "Potential sensitive content found. Review before publishing." >&2
    exit 1
  fi
else
  if grep -R -n -E -i "$pattern" "$repo_root" --exclude-dir=.git --exclude=check-sensitive.sh --exclude=.env.example; then
    echo "Potential sensitive content found. Review before publishing." >&2
    exit 1
  fi
fi
